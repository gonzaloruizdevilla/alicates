...
describe('addIndex', () => {
  describe('works with unary functions like `map`', () => {
    ...
    it('passing the index as a second parameter to the callback', () => {
      assert.deepEqual(
        mapIndexed(addIndexParam, [8, 6, 7, 5, 3, 0, 9]),
        [8, 7, 9, 8, 7, 5, 15] // [8 + 0, 6 + 1...]
      );
    });

    it('passing the entire list as a third parameter to the callback', () => {
      assert.deepEqual(
        mapIndexed(squareEnds, [8, 6, 7, 5, 3, 0, 9]),
        [64, 6, 7, 5, 3, 0, 81]
      );
    });

    it('acting as a curried function', () => {
      let makeSquareEnds = mapIndexed(squareEnds);
      assert.deepEqual(
        makeSquareEnds([8, 6, 7, 5, 3, 0, 9]),
        [64, 6, 7, 5, 3, 0, 81]
      );
    });
  });
  ...
  describe('works with functions like `all` that do not typically have index applied', () => {
    let allIndexed = addIndex(all);
    let superDiagonal = allIndexed(gt);
    it('passing the index as a second parameter', () => {
      assert.strictEqual(
        superDiagonal([8, 6, 5, 4, 9]),
        true // 8 > 0, 6 > 1, 5 > 2, 4 > 3, 9 > 5
      );
      assert.strictEqual(
        superDiagonal([8, 6, 1, 3, 9]),
        false //  1 !> 2, 3 !> 3
      );
    });
  });
  ...
  describe('works with arbitrary user-defined functions', () => {
    let mapFilter = function(m, f, list) {
      return filter(compose(f, m), list);
    };
    let mapFilterIndexed = addIndex(mapFilter);
